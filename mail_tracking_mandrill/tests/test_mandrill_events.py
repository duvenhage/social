...
class TestMandrill(TransactionCase):
    ...
    def setUp(self):
        ...
        self.event_open = {
            "mandrill_events": [{
                ...
                "msg": {
                    ...
                    "clicks": [],
                    "metadata": {
                        "tracking_email_id": f"{self.tracking_email.id}"
                    },
                    "resends": [],
                    "state": "sent",
        ...
        self.event_deferral = {"mandrill_events": [{
            "msg": {
                "sender": "username01@example.com",
                "tags": [],
                "smtp_events": [
        ...
        self.event_reject = {"mandrill_events": [{
            "msg": {
                "_id": self.mandrill_message_id,
                "subaccount": None,
                "tags": [],
        ...
        self.event_unsub = {"mandrill_events": [{
            "msg": {
                "_id": self.mandrill_message_id,
                "subaccount": None,
                "tags": [],
        ...
        self.event_spam = {"mandrill_events": [{
            "msg": {
                "sender": "username01@example.com",
                "tags": [],
                "smtp_events": [],
        ...
        self.event_click = {"mandrill_events": [{
    ...
    def test_process_hard_bounce_event(self):
        ...
        self.env["mail.tracking.email"].event_process(None, self.event_hard_bounce, self.metadata)

        event_count_2 = self.env["mail.tracking.event"].search_count(
            [("tracking_email_id", "=", self.tracking_email.id)])

        self.assertGreater(event_count_2, event_count_1)
        self.assertEqual(self.tracking_email.tracking_event_ids[1].event_type, "hard_bounce")

    def test_process_soft_bounce_event(self):
        event_count_1 = self.env["mail.tracking.event"].search_count(
            [("tracking_email_id", "=", self.tracking_email.id)])

        self.env["mail.tracking.email"].event_process(None, self.event_soft_bounce, self.metadata)

        event_count_2 = self.env["mail.tracking.event"].search_count(
            [("tracking_email_id", "=", self.tracking_email.id)])

        self.assertGreater(event_count_2, event_count_1)
        self.assertEqual(self.tracking_email.tracking_event_ids[1].event_type, "soft_bounce")

    def test_process_deferral_event(self):
        event_count_1 = self.env["mail.tracking.event"].search_count(
            [("tracking_email_id", "=", self.tracking_email.id)])

        self.env["mail.tracking.email"].event_process(None, self.event_deferral, self.metadata)

        event_count_2 = self.env["mail.tracking.event"].search_count(
            [("tracking_email_id", "=", self.tracking_email.id)])

        self.assertGreater(event_count_2, event_count_1)
        self.assertEqual(self.tracking_email.tracking_event_ids[1].event_type, "deferral")

    def test_process_reject_event(self):
        event_count_1 = self.env["mail.tracking.event"].search_count(
            [("tracking_email_id", "=", self.tracking_email.id)])

        self.env["mail.tracking.email"].event_process(None, self.event_reject, self.metadata)
    ...
    def test_process_unsub_event(self):
        ...
        self.env["mail.tracking.email"].event_process(None, self.event_unsub, self.metadata)
    ...
    def test_process_spam_event(self):
        ...
        self.env["mail.tracking.email"].event_process(None, self.event_spam, self.metadata)
    ...
    def test_process_open_event(self):
        ...
        self.env["mail.tracking.email"].event_process(None, self.event_open, self.metadata)
    ...
    def test_process_click_event(self):
        ...
        self.env["mail.tracking.email"].event_process(None, self.event_click, self.metadata)
