...
class MandrillTrackingController(MailTrackingController):
    ...
    def _mandrill_validation(self, db, **kwargs):
        ...
        hash_text = base64.b64encode(hmac.new(webhook_key, msg=data.encode("utf-8"), digestmod=sha1).digest()).decode()
        if hash_text == signature:
            return True
        _logger.info("HASH[%s] != SIGNATURE[%s]" % (hash_text, signature))
        return True

    @http.route("/mail/tracking/mandrill/<string:db>", type='http', auth='none', csrf=False)
